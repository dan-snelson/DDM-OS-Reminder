...
function resolvePaddedEnforcementDateForCandidate() {
    ...
    local latestPaddedLineEpoch=0
    local paddedDateRaw=""
    local paddedEpoch=""
    local nowEpoch=""
    local parsedSignature=""

    if ! ddmLogTimestampToEpoch "${declarationTimestamp}"; then
        return 1
    fi
    ...
    for line in "${ddmRecentInstallLogWindow[@]}"; do
        ...
        if [[ "${line}" == *"EnforcedInstallDate:"* ]] && parseDDMDeclarationFromLine "${line}"; then
            ...
            if [[ "${parsedSignature}" != "${declarationSignature}" ]]; then
                return 1
            fi
        fi

        if [[ "${line}" == *"setPastDuePaddedEnforcementDate is set: "* ]] && (( lineEpoch >= latestPaddedLineEpoch )); then
            latestPaddedLine="${line}"
            latestPaddedLineEpoch="${lineEpoch}"
        fi
    done

    if [[ -n "${latestPaddedLine}" ]]; then
        paddedDateRaw="${latestPaddedLine#*setPastDuePaddedEnforcementDate is set: }"
    fi

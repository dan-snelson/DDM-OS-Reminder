...
> You can also launch with just a dragged `.plist` path; `assemble.zsh` will auto-enable interactive mode and infer both the RDNN and deployment mode from that plist when the filename is unambiguous.
> Older supported plists without a `-dev`, `-test`, or `-prod` filename suffix can still import successfully, but `assemble.zsh` will prompt for deployment mode.

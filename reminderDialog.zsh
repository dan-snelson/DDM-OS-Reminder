...
declare -A preferenceConfiguration=(
    ...
    ["messageLocalizedDe"]="string|**Ein erforderliches macOS-{titleMessageUpdateOrUpgrade:l} ist jetzt verfügbar**<br><br>Hallo {loggedInUserFirstname}!<br><br>Bitte führe das {titleMessageUpdateOrUpgrade:l} auf macOS **{ddmVersionString}** durch, damit dein Mac sicher bleibt und den Richtlinien entspricht.{updateReadyMessage}<br><br>Um das {titleMessageUpdateOrUpgrade:l} jetzt zu starten, klicke auf **{button1text}**, folge den Hinweisen auf dem Bildschirm und klicke anschliessend auf **{softwareUpdateButtonText}**.<br><br>Wenn du das {titleMessageUpdateOrUpgrade:l} jetzt nicht durchführen kannst, klicke auf **{button2text}**, um später erinnert zu werden (diese Option ist kurz vor der Frist deaktiviert).<br><br>{deadlineEnforcementMessage}{excessiveUptimeWarningMessage}{diskSpaceWarningMessage}{supportAssistanceMessage}"
